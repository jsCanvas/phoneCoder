...
Expo React Native + react-native-web client for the **phoneBot** AI coding
service (`/phoneBot`). Replaces the original phoneClaw integration.

## Features

The app is six-tab and maps 1:1 with the phoneBot HTTP API surface:

| Tab        | What you can do                                                                       | phoneBot endpoints used                                                                 |
| ---------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| Settings   | Configure phoneBot base URL; CRUD multiple OpenAI-compatible model configs; activate or test any of them. | `GET/POST/PATCH/DELETE /model-configs`, `POST /model-configs/:id/activate`, `POST /model-configs/:id/test` |
...
`phonebot.client.settings` key.

## Streaming protocol

phoneBot emits *named* SSE events on the chat endpoint
...
On a phone, point Settings → "phoneBot API Base URL" at your server's LAN
...
src/
  ...
  types/api.ts                # types mirroring phoneBot DTOs
...
- Replaces the singleton model config with the phoneBot multi-config CRUD.
